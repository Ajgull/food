...
def test_profile_customer_creation(django_user_model):
    ...
    assert profile.phone.as_e164 == '+79876543210'
...
def test_dish_creation(django_user_model):
    ...
    profile = Profile.objects.create(
        first_name='test', second_name='test', user=user, email='test@test.com', phone='+79876543210', role='restaurant'
    )
    restaurant = Restaurant.objects.create(name='test', address='test test', description='Nice', profile=profile)
    dish = Dish.objects.create(
        name='test', description='test test', price=Decimal('10.50'), image='test.png', restaurant=restaurant
    )
...
def test_restaurant_creation(user_with_profile_restaurant):
    ...
    restaurant = Restaurant.objects.create(
        name='test', address='test address', description='test description', profile=profile
    )
...
def test_comment_creation(user_with_profile_customer):
    user, profile = user_with_profile_customer
    restaurant = Restaurant.objects.create(
        name='Test Restaurant', address='123 Test Ave', description='A test restaurant', profile=profile
    )
    dish = Dish.objects.create(
        name='Test Dish',
    ...
    comment = Comment.objects.create(text='Great dish!', author=user, dish=dish)
...
def test_order_and_orderitem_creation(user_with_profile_customer):
    user, profile = user_with_profile_customer
    restaurant = Restaurant.objects.create(
        name='Order Restaurant', address='234 Test Blvd', description='Another test restaurant', profile=profile
    )
    ...
    order_item = OrderItem.objects.create(order=order, dish=dish, quantity=3)
...
def test_like_creation(user_with_profile_customer):
    user, profile = user_with_profile_customer
    restaurant = Restaurant.objects.create(
        name='Like Restaurant', address='345 Test St', description='Restaurant for likes', profile=profile
    )
    dish = Dish.objects.create(
        name='Liked Dish',
    ...
    like = Like.objects.create(dish=dish, profile=user)
